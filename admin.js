...
async function loadSettingsToUI() {
    ...
    if (settings) {
        // API Anahtarları artık kod ve Vercel tarafında, UI'dan kaldırıldı.
...
async function verifyGrokKey(key) {
    try {
        const resp = await fetch('/api/openrouter', {
            ...
            body: JSON.stringify({
                apiKey: key,
                requestBody: {model: "google/gemini-flash-1.5", messages: [{role: "user", content: "hi"}], max_tokens: 1}
            })
        });
        return resp.ok;
    } catch(e) { return false; }
...
async function updateSystemSettings() {
    ...
    if (window.useCloud) {
        ...
        if (adminKey) {
            localDb[adminKey].password = newPass;
            localStorage.setItem('manga_saas_db', JSON.stringify(localDb));
        }
    }
